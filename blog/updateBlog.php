...
if (!isset($_SESSION['user_session']) && empty($_SESSION['user_session'])) {
  ...
  if (isset($_POST['editBlog'])) {
    ...
    if(empty($btitle_error) && empty($boverview_error) && empty($bcontent_error) && empty($bdate_error)){
        $blog = new Blog();
        if($blog->updateBlog($btitle, $boverview, $bcontent, $bdate, $_SESSION['blog']))
        {
          $edit_error = "Successful";  
        }else{
         echo "Wrong Details!";
        }
    }
...
<body>
  ...
  <div class="div-wrapper">
    ...
    <form action="" method="POST">
      ...
      <div class="form-group">
        <button style="border: none; font-size: 18px;" name="editBlog" class="link-btn">Edit Blog</button>
        <span class="alert"> <?php echo $edit_error; ?></span>
      </div>
      <div class="form-group">
